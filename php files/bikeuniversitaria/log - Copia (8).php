			...
			<?php
				...
				while($linhalog = mysql_fetch_array($resultlog))
				{
					...
					if (!empty($_SERVER['QUERY_STRING']))//caso nao seja passado nada via get
					{
						...
						if (!empty($_GET['matricula'])) $buscausuario .= " AND matricula = ".$_GET['matricula'];
						if (!empty($_GET['cartao'])) $buscausuario .= " AND cartao = ".$_GET['cartao'];
						if (!empty($_GET['email'])) $buscausuario .= " AND email = ".$_GET['email'];
						if (!empty($_GET['telefone'])) $buscausuario .= " AND telefone = ".$_GET['telefone'];
